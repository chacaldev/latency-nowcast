...
timestr = "%d %b %Y %H:%M:%S"
model_folder = "models/"
result_folder = "results/lstm_daily/rtt/"
skip_existent_model = True

logging.basicConfig(filename="diff_lstm_week_rtt_log.txt", level=logging.INFO)
...
def generate_model(data, model_name):
    ...
    for i in range(0,n-7):
        ...
        if (skip_existent_model and exists(model_folder + model_name + str(i) +'.h5')):
            ...
            continue
        m = idx[i:i+8]
        df_train = dt[dt['ord_date'].between(m[0], m[6])]
        df_test = dt[dt['ord_date'] == m[7]]
    
        dfc = dt[dt['ord_date'].between(m[0], m[7])]
...
for seg in segments:
    generate_model('Datasets/rtt_parquet/rtt-'+seg+'_diff1pre.gzip', 'rtt-diff'+seg)
